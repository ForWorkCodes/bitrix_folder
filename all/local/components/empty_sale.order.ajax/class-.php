		...
		try {
            ...
            foreach ($basketItems as $arItem)
            {
                ...
                if ($r = $res->GetNext())
                {
                    ...
                    $isOffer = CCatalogSku::GetProductInfo($r['ID']);
                    if (is_array($isOffer)) { // Является ли ID торговым предложением

                    	$realElement = CIBlockElement::GetByID($isOffer["ID"]); // Получаем ID товара по ID его торгового предложения
                    	if ($realImg = $realElement->GetNext()) {
	                    	if (!empty($realImg['DETAIL_PICTURE']))
		                        $img = $realImg['DETAIL_PICTURE'];
		                    elseif (!empty($realImg['PREVIEW_PICTURE']))
		                        $img = $realImg['PREVIEW_PICTURE'];
		                    else
		                        $img = SITE_TEMPLATE_PATH . '/images/wom.png';
		                    $linkCart = $realImg['DETAIL_PAGE_URL'];

                    	}

                    }else { // Если, это, всетаки товар, а не торговое предложение
                    ...
                    // Если товар это торговое предложение
                    if (is_array($isOffer)) {
                    	$offers = CIBlockElement::GetList(
                    		[],
                    		['IBLOCK_ID' => $r['IBLOCK_ID'], 'ID' => $r['ID']],
                    		false,
                    		false,
                    		[]
                    	);
                    	...
                    	while ($offer = $offers->GetNextElement()) {
                    		$prop = $offer->GetProperties();
                    		foreach ($prop as $keyProp => $arProp) {
                    			if ($keyProp == 'COLOR') { // Жестко проверяем на код COLOR
                    				$ID = 2; // Жестко задаем ID хайлоад с каритнками
				                    ...
				                    $arFilter = Array(
				                                    Array(
				                                        "LOGIC"=>"AND",
				                                        Array(
				                                            "UF_XML_ID"=> $arProp['VALUE'] //НАШ МАССИВ С ЦВЕТАМИ
				                                            )
				                                        )
				                                    );
				                    ...
				                    {
				                        $arProp['VALUE'] = CFile::GetPath($res["UF_FILE"]); //ПОЛУЧАЕМ ПУСТЬ К КАРТИНКЕ
				                    }
                    			}
                    			$arOfferShow[] = ['NAME' => $arProp['NAME'], 'CODE' => $keyProp, 'VALUE' => $arProp['VALUE']];
                    		}
                    	}
                    }
			...
			$this->getCurrency = $this->order->getCurrency();
