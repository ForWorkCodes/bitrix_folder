...
class order_from_turn extends main_class
{
	...
	*/
	...
	{
		$Iblock_cart_month = Option::get('plastilin.settings', "Iblock_cart_month");
		...
		$params = [
			'DESCRIPTION' => Option::get('plastilin.settings', "subs_desc"),
			'PAYMENT' => Option::get('plastilin.settings', "subs_payment"),
			'USER_ID' => $user_id,
			'PRICE_ID' => Option::get('plastilin.settings', "price_turn"),
			'DELIVERY' => Option::get('plastilin.settings', "subs_delivery"),
			'PROPS' => $PROPS,
			'IS_NEW' => 'Y'
		];

		$obOrder = new \plastilin\settings\Order($params, $arProducts);
	...
	private function GetArrayProducts($user_id)
	{
		...
		CModule::IncludeModule('plastilin.settings');
		$arItemsMonth = \plastilin\settings\MainPlastilin::GetElements($arMonth['ID'], $this->iblock_trun); // Список элементов в первом месяце
		...
		if (is_array($arItemsMonth) && count($arItemsMonth) > 0)
		{
			foreach ($arItemsMonth as $arItemMonth)
			{
				...
				$arItems[] = \plastilin\settings\MainPlastilin::GetElement($arItemsId); // Список товаров
			}
		}
