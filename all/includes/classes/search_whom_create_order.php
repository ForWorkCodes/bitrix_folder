...
class search_whom_create_order extends main_class
{
	...
	*/
	...
	{
		$this->turn_before_date = Option::get('plastilin.settings', "turn_before_date");
	...
	private function CheckItem($user_id, $arMonths)
	{
		...
		{
			$arMonth = array_shift($arMonths);
			$arItems = \plastilin\settings\MainPlastilin::GetElements($arMonth['ID']);
			if (is_array($arItems) && count($arItems) > 0)
			{
				foreach ($arItems as $arItem)
				{
					$cart = \plastilin\settings\MainPlastilin::GetElement($arItem['PROPERTIES']['ID_ITEM']['VALUE']);
					if (empty($cart))
					{
						$order_from_turn = new order_from_turn();
